  ...
  const float Kp[3], short kumar_variant, float rates_des[3], float *thrust)
{
  ...
  x_C_des[0] = (real32_T)cos(yaw);
  x_C_des[1] = (real32_T)sin(yaw);
  y_C_des[0] = (real32_T)cos((yaw + 1.57079637F));
  y_C_des[1] = (real32_T)sin((yaw + 1.57079637F));
  if (kumar_variant != 0) {
  ...
  // angle_err1 = r1(4);
  // angle_err2 = r2(4);
  if ((real32_T)fabs((mtrc - 3.0F)) <= 1.0E-6F) {
    f0 = 0.0F;
  } else if ((real32_T)fabs((mtrc + 1.0F)) <= 1.0E-6F) {
    f0 = 3.14159274F;
  } else {
    f0 = (real32_T)acos(((mtrc - 1.0F) / 2.0F));
  }

  if ((real32_T)fabs((b_mtrc - 3.0F)) <= 1.0E-6F) {
    f1 = 0.0F;
  } else if ((real32_T)fabs((b_mtrc + 1.0F)) <= 1.0E-6F) {
    f1 = 3.14159274F;
  } else {
    f1 = (real32_T)acos(((b_mtrc - 1.0F) / 2.0F));
  }
